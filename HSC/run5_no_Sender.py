...
import serial
#%%  Subprocess for operating the sender radio code 

#Sender_Code = multiprocessing.Process(target=lambda: exec(open("Sender.py").read()))
...
def record(channel):
    global bash_process
    global RTK_GNSS_code
    
    if GPIO.input(Toggle_switch) == GPIO.HIGH:
        RTK_GNSS_code = multiprocessing.Process(target=lambda: exec(open("RTK_GNSS.py").read()))
...
#            RTK_GNSS_code.stop()
            tty_device_RTK_GNSS ='/dev/ttyACM2'
            close_serial_connection(tty_device_RTK_GNSS)
